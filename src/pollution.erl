...
%% API
-export([createMonitor/0,addValue/5,addStation/3,removeValue/4,getOneValue/4,getStationMean/3,getDailyMean/3,getHourlyMean/4]).
...
addValue(Name,Date,Type,Value,#{names := Names, measures := Measures } = Monitor) ->
  case maps:is_key(Name,Names) of
    true -> case lists:filter(fun(X) -> addingHelper(Name,Date,Type,X) end , Measures) of
              [] -> Monitor#{measures => [#measure{name = Name,type = Type,value = Value,date = Date}|Measures]};
              [H|T] -> {error,"There is a record of this Type,Date,and assigned to this Station already!"}
            end;
    false -> {error,"There is no station with such name!"} %this one won't actually ever happen, but it needs to be here :/
  end.

addingHelper(Name,Date,Type,X) ->
  (X#measure.date == Date andalso X#measure.type == Type andalso X#measure.name == Name).
...
getDailyMean(Type,{_,_,_} = Day, #{measures := Measures})->
...
getHourlyMean(Type,Hour,Name,#{measures := Measures}) ->
  case meanHelper(lists:filter(fun(X) -> X#measure.type == Type andalso element(1,element(2,X#measure.date)) == Hour andalso X#measure.name == Name end,Measures)) of
